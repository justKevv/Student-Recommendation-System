    ...
    <div class="flex flex-col gap-4 lg:flex-row">
        ...
        @if($media)
            <div class="flex-shrink-0 lg:w-1/3">
                <img src="{{ $media }}" alt="{{ $title }}" class="object-cover w-full h-48 rounded-lg border border-gray-200 lg:h-32">
            </div>
        @endif
            ...
            <div class="flex justify-between items-start">
                ...
                @if($projectUrl || $githubUrl)
                    <div class="flex gap-2 ml-4">
                        @if($projectUrl)
                            <a href="{{ $projectUrl }}" target="_blank" class="text-redgoon hover:text-redgoon2 text-sm font-medium font-['Poppins']">
                        ...
                        @endif
                        @if($githubUrl)
                            <a href="{{ $githubUrl }}" target="_blank" class="text-gray-600 hover:text-gray-800 text-sm font-['Poppins']">
                                <svg class="inline w-4 h-4" fill="currentColor" viewBox="0 0 20 20">
