...
<div id="hs-vertically-centered-modal-research-interest-{{ $userId }}" class="hidden overflow-y-auto overflow-x-hidden fixed top-0 pointer-events-none hs-overlay size-full start-0 z-80" role="dialog" tabindex="-1" aria-labelledby="hs-vertically-centered-modal-label-{{ $userId }}">
    <div class="hs-overlay-open:mt-7 hs-overlay-open:opacity-100 hs-overlay-open:duration-600 mt-0 opacity-0 ease-out transition-all sm:max-w-lg sm:w-full m-3 sm:mx-auto min-h-[calc(100%-56px)] flex items-center">
      <div class="flex flex-col w-full bg-white rounded-xl border border-gray-200 pointer-events-auto shadow-2xs">
        ...
        <div class="overflow-y-auto p-4">
          <div class="mb-6">
             ...
             <div id="research-interests-display-{{ $userId }}" class="flex flex-wrap gap-3">
               @foreach($researchInterests as $interest)
                 <span class="research-interest-item group relative px-4 py-2 bg-skillbg outline-2 outline-skilloutline text-main text-sm font-medium font-['Poppins'] rounded-[15px]">
                   {{ $interest }}
          ...
          <div class="flex gap-2">
            <input type="text" id="research-input-{{ $userId }}" class="flex-1 px-3 py-2 rounded-md border border-gray-300 focus:outline-none focus:ring-1 focus:ring-yellowgoon" placeholder="Enter research interest" onkeypress="if(event.key==='Enter') addResearchInterestToModal({{ $userId }})" autoFocus="">
            <button type="button" class="px-4 py-2 rounded-md bg-main text-yellowgoon hover:bg-gray-600" onclick="addResearchInterestToModal({{ $userId }})">
        ...
        <div class="flex gap-x-2 justify-end items-center px-4 py-3 border-t border-gray-200">
          <button type="button" class="inline-flex gap-x-2 items-center px-3 py-2 text-sm font-medium text-gray-800 bg-white rounded-lg border border-gray-200 shadow-2xs hover:bg-gray-50 focus:outline-hidden focus:bg-gray-50 disabled:opacity-50 disabled:pointer-events-none" data-hs-overlay="#hs-vertically-centered-modal-research-interest-{{ $userId }}">
            Close
          </button>
          <button type="button" class="inline-flex gap-x-2 items-center px-3 py-2 text-sm font-medium rounded-lg border border-transparent transition bg-main text-yellowgoon hover:bg-gray-700 focus:outline-hidden disabled:opacity-50 disabled:pointer-events-none" onclick="saveResearchInterests({{ $userId }})">
            Save
          </button>
        </div>
      </div>
...
function updateResearchInterestsDisplay(userId) {
  ...
  if (tempResearchInterests[userId].length === 0) {
    displayDiv.innerHTML = `<div id="no-research-interests-${userId}" class="text-sm italic text-gray-400">No research interests added yet</div>`;
  } else {
    tempResearchInterests[userId].forEach(interest => {
      const span = document.createElement('span');
      span.className = 'research-interest-item group relative px-4 py-2 bg-skillbg outline-2 outline-skilloutline text-main text-sm font-medium font-[\'Poppins\'] rounded-[15px]';
      span.innerHTML = `
        ${interest}
        <button type="button" class="hidden absolute -top-1 -right-1 w-4 h-4 text-xs leading-none text-white rounded-full bg-redgoon group-hover:block" onclick="removeResearchInterestFromModal(this, ${userId})">
          ×
...
document.addEventListener('DOMContentLoaded', function() {
  ...
  const modalTrigger = document.querySelector('[data-hs-overlay="#hs-vertically-centered-modal-research-interest-{{ $userId }}"]');
  if (modalTrigger) {
