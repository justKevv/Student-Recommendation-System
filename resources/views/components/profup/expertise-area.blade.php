...
<div class="w-full bg-white rounded-[30px] shadow-[0px_0px_19.899999618530273px_0px_rgba(0,0,0,0.10)] p-6 min-h-[120px]">
    ...
    <div class="flex flex-wrap gap-3 items-start">
        @foreach ($expertiseAreas as $area)
            <span
                class="px-4 py-2 bg-skillbg outline-2 outline-skilloutline text-main text-sm font-medium font-['Poppins'] rounded-[15px]">{{ $area }}</span>
        @endforeach
        @if (auth()->user()->role != 'admin')
            <button
                ...
                aria-controls="hs-vertically-centered-modal-expertise-area-{{ $userId }}"
                data-hs-overlay="#hs-vertically-centered-modal-expertise-area-{{ $userId }}">
...
<div id="hs-vertically-centered-modal-expertise-area-{{ $userId }}"
    ...
    <div
        ...
        <div class="flex flex-col w-full bg-white rounded-xl border border-gray-200 pointer-events-auto shadow-2xs">
            ...
            <div class="overflow-y-auto p-4">
                <div class="mb-6">
                    ...
                    <div id="expertise-area-display-{{ $userId }}" class="flex flex-wrap gap-3">
                        @foreach ($expertiseAreas as $area)
                            <span
                                class="expertise-area-item group relative px-4 py-2 bg-skillbg outline-2 outline-skilloutline text-main text-sm font-medium font-['Poppins'] rounded-[15px]">
                                {{ $area }}
                                <button type="button"
                                    class="hidden absolute -top-1 -right-1 w-4 h-4 text-xs leading-none text-white rounded-full bg-redgoon group-hover:block"
                                    onclick="removeExpertiseAreaFromModal(this, {{ $userId }})">
                                    ×
                                </button>
                            </span>
                        @endforeach
                        @if (empty($expertiseAreas))
                            <div id="no-expertise-areas-{{ $userId }}" class="text-sm italic text-gray-400">No
                                expertise areas added yet</div>
                        @endif
                    </div>
                </div>
                <div class="flex gap-2">
                    <input type="text" id="expertise-input-{{ $userId }}"
                        class="px-3 py-2 w-full rounded-md border border-gray-300 focus:outline-none focus:ring-1 focus:ring-yellowgoon"
                        placeholder="Enter expertise area"
                        onkeypress="if(event.key==='Enter') addExpertiseAreaToModal({{ $userId }})"
                        autoFocus="">
                    <button type="button" class="px-4 py-2 rounded-md bg-main text-yellowgoon hover:bg-gray-600"
                        onclick="addExpertiseAreaToModal({{ $userId }})">
            ...
            <div class="flex gap-x-2 justify-end items-center px-4 py-3 border-t border-gray-200">
                ...
                <button type="button"
                    class="inline-flex gap-x-2 items-center px-3 py-2 text-sm font-medium rounded-lg border border-transparent bg-main text-yellowgoon hover:bg-gray-700 focus:outline-hidden focus:bg-gray-700 disabled:opacity-50 disabled:pointer-events-none"
...
<script>
    ...
    function updateExpertiseAreasDisplay(userId) {
        ...
        if (tempExpertiseAreas[userId].length === 0) {
            displayDiv.innerHTML =
                `<div id="no-expertise-areas-${userId}" class="text-sm italic text-gray-400">No expertises area added yet</div>`;
        } else {
            tempExpertiseAreas[userId].forEach(expertise => {
                const span = document.createElement('span');
                span.className =
                    'expertise-area-item group relative px-4 py-2 bg-skillbg outline-2 outline-skilloutline text-main text-sm font-medium font-[\'Poppins\'] rounded-[15px]';
                span.innerHTML = `
                    ${expertise}
