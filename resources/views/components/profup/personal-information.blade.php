...
<div class="bg-white rounded-[30px] shadow-[0px_0px_19.899999618530273px_0px_rgba(0,0,0,0.10)] p-6">
    <div class="flex justify-between items-center mb-6">
        ...
        @if (auth()->user()->role != 'admin')
            <button
                class="inline-flex items-center gap-2 px-4 py-2 rounded-[30px] text-sm font-medium bg-main text-white hover:bg-gray-700 transition-colors duration-200"
                aria-haspopup="dialog" aria-expanded="false" aria-controls="hs-personal-modal-{{ $userId }}"
                data-hs-overlay="#hs-personal-modal-{{ $userId }}">
        ...
        <div class="flex flex-col w-full bg-white rounded-xl border border-gray-200 pointer-events-auto shadow-2xs">
            <div class="flex justify-between items-center px-4 py-3 border-b border-gray-200">
                ...
                <button type="button"
                    ...
                    aria-label="Close" data-hs-overlay="#hs-personal-modal-{{ $userId }}">
            ...
            <form action="{{ route('profile.edit') }}" method="post">
                ...
                <div class="overflow-y-auto p-4 space-y-4">
                    <div>
                        <label for="phone-input-{{ $userId }}"
                            class="block mb-2 text-sm font-medium text-gray-700">
                            Phone Number
                        </label>
                        <input type="tel" id="phone-input-{{ $userId }}" name="phone"
                            class="px-3 py-2 w-full rounded-md border border-gray-300 focus:outline-none focus:ring-1 focus:ring-yellowgoon @error('phone') border-red-500 @enderror"
                            placeholder="Enter phone number" value="{{ old('phone', $phoneNumber) }}">
                        @error('phone')
